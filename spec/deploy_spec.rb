...
describe 'DeployController' do
  ...
  before(:each) do
    ...
    @other_tag_generator = spy('other_tag_generator')
  end

  describe 'environment_choice' do
    before(:each) do
      @deploy = DeployController.new('develop', @git_helper, @user_comms, @develop_tag_generator, @other_tag_generator)
    ...
    describe 'test' do
      ...
      before (:each) do
        @deploy = DeployController.new('test', @git_helper, @user_comms, @develop_tag_generator, @other_tag_generator)
        allow(@git_helper).to receive(:get_tags_for_this_commit)
      end
        ...
        context 'when there is a develop tag, dev-v0.1.5 on the commit' do
          before(:each) do
            ...
            allow(@other_tag_generator).to receive(:next_tag).and_return('test-v0.1.5')
